...
export default function FromTable({data, dayKey}) {
  const {setCompletedExercises} = useCompletedExercises()
  ...
  useEffect(() => {
    const allCompleted = data.every((exercise) => allSetsFilled(exercise.id))
          ...
          <tbody>
            {data.map((el) => (
              <tr
                ...
                <th
                  scope="row"
                  className="h-full flex flex-col items-start justify-center gap-2 px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                  {el.title}
